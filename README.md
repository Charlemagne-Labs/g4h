...
3. **Eval + demo** — `src/infer.py`, eval on the held-out test split, ad-hoc predict CLI. **Runbook: [`docs/phase3_eval.md`](docs/phase3_eval.md)**.
...
- [x] **Phase 3 eval run complete** (2026-05-11) — 324 held-out test rows: **100% accuracy** overall and **100% accuracy on the 265-row novel-text subset** (text never seen during training). The 18% duplicate subset (59 rows) also at 100%. Defensible generalization number for the hackathon writeup.
